...
import { EmptyState, EntityCard, FilterChip, NavigationLink, PlatformShell } from '@/components/platform/PlatformScaffold'
...
export function Home(props: HomeProps) {
  ...
  useEffect(() => {
    ...
    setLoadError('')
    const loadHome = () => {
      void platformApi.fetchHome('characters', searchQuery, characterFilter, worldFilter)
        .then((home) => {
          if (!mounted) return
          setHomePayload(home)
        })
  ...
  }, [characterFilter, reloadVersion, searchQuery, worldFilter])
  ...
  useEffect(() => {
    ...
    setRecentLoadError(false)
    void platformApi.fetchRecentRooms({ limit: 3, includeMessages: false })
      .then((recent) => { if (mounted) setRecentRooms(recent.items.slice(0, 3)) })
      .catch(() => { if (mounted) setRecentLoadError(true) })
    return () => { mounted = false }
  }, [recentReloadVersion, user])
      ...
      <div className="mx-auto max-w-[1080px] space-y-12">
        ...
        <section aria-labelledby="character-title" className="space-y-4">
          ...
          {isLoading ? <CardSkeletons type="character" count={2} /> : characters.length === 0 ? <EmptyState title={searchQuery ? '검색 결과가 없습니다' : '공개된 캐릭터가 없습니다'} description={searchQuery ? '다른 이름이나 태그로 검색해 보세요.' : '직접 캐릭터를 만들 수 있습니다.'} action={!searchQuery ? <Button onClick={() => user ? onNavigate('/create/character') : onAuthRequest()} className="bg-[#d43a34] shadow-none hover:bg-[#c9342f]">캐릭터 만들기</Button> : null} /> : (
            <div data-catalog-grid="characters" className="grid grid-cols-2 gap-x-3 gap-y-8 sm:gap-x-5">{characters.map((item, index) => <EntityCard key={item.id} item={item} priority={index === 0} selected={selectedCharacter?.id === item.id} href={`/characters/${item.slug}`} onNavigate={onNavigate} onSelect={() => onSelectEntity(item)} />)}</div>
          )}
        </section>
        ...
        <section aria-labelledby="world-title" className="space-y-4">
          ...
          {isLoading ? <CardSkeletons type="world" count={2} /> : worlds.length === 0 ? <EmptyState title={searchQuery ? '검색 결과가 없습니다' : '공개된 월드가 없습니다'} description={searchQuery ? '다른 이름이나 태그로 검색해 보세요.' : '직접 월드를 만들 수 있습니다.'} action={!searchQuery ? <Button onClick={() => user ? onNavigate('/create/world') : onAuthRequest()} className="bg-[#d43a34] shadow-none hover:bg-[#c9342f]">월드 만들기</Button> : null} /> : (
            <div data-catalog-grid="worlds" className="grid grid-cols-2 gap-x-3 gap-y-8 sm:gap-x-5">{worlds.map((item, index) => <EntityCard key={item.id} item={item} priority={index === 0} selected={selectedWorld?.id === item.id} href={`/worlds/${item.slug}`} onNavigate={onNavigate} onSelect={() => onSelectEntity(item)} />)}</div>
          )}
        </section>
        </>}
        ...
        {user ? (
          <section aria-labelledby="recent-title" className="space-y-4">
            <div className="flex items-center justify-between"><h2 id="recent-title" className="text-xl font-black tracking-[-0.04em] text-[#171717]">이어서 대화하기</h2>{recentRooms.length ? <NavigationLink path="/recent" onNavigate={onNavigate} className="flex items-center text-xs font-bold text-[#c9342f]">전체 보기<ChevronRight className="size-4" /></NavigationLink> : null}</div>
            {recentLoadError ? <EmptyState title="대화 목록을 불러오지 못했습니다" description="현재 화면의 다른 내용은 그대로입니다. 다시 불러와 주세요." action={<Button variant="outline" onClick={() => setRecentReloadVersion((value) => value + 1)}>다시 불러오기</Button>} /> : recentRooms.length === 0 ? <EmptyState title="아직 시작한 대화가 없습니다" description="캐릭터를 선택해 대화를 시작하세요." /> : <div className="grid gap-3 lg:grid-cols-3">{recentRooms.map((room) => <NavigationLink key={room.id} path={`/rooms/${room.id}`} onNavigate={onNavigate} className="flex items-center gap-3 rounded-xl border border-[#e7e7e7] bg-white p-3 text-left transition hover:border-[#c6c6c6]"><img src={room.character.avatarImageUrl || room.character.coverImageUrl} alt="" decoding="async" className="size-12 rounded-lg object-cover" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-[#171717]">{room.character.name}{room.world ? ` × ${room.world.name}` : ''}</p><p className="mt-1 truncate text-xs text-[#777777]">{room.state.currentSituation}</p></div><MessageSquareMore className="size-4 text-[#c9342f]" /></NavigationLink>)}</div>}
          </section>
        ) : null}
      </div>
    </PlatformShell>
