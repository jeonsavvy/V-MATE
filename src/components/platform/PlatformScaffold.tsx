...
export function EmptyState({ title, description, action }: { title: string; description?: string; action?: ReactNode }) {
  return <div className="rounded-xl border border-dashed border-[#d8d8d8] bg-[#ffffff] px-5 py-10 text-center"><p className="text-base font-bold text-[#171717]">{title}</p>{description ? <p className="mt-2 text-sm leading-6 text-[#6f6f6f]">{description}</p> : null}{action ? <div className="mt-4">{action}</div> : null}</div>
}

export function LoadingState({ label = '불러오는 중…' }: { label?: string }) {
  return <div role="status" aria-live="polite" className="flex min-h-36 items-center justify-center gap-2 text-sm text-[#6f6f6f]"><Loader2 className="size-4 animate-spin" />{label}</div>
}
