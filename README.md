...
starter 콘텐츠는 owner 계정의 17+ 확인 후 `supabase/operations/publish_starter_content.sql`을 별도 승인된 SQL Editor 세션에서 실행합니다. 스크립트는 간결한 공식 테스트 픽스처인 캐릭터 `캐릭터A`, `캐릭터B`와 월드 `월드A`, `월드B`를 upsert합니다. 기존 `CharacterA`, `World1`, `안개 낀 항구 도시`와 이전 스타터는 삭제하지 않고 숨깁니다.
