with release_objects(kind, identity, definition) as (
  ...
  select 'routine', namespace_record.nspname || '.' || routine_record.proname || '('
    ...
    jsonb_build_array(pg_get_functiondef(routine_record.oid), routine_record.proacl)::text
  from pg_proc routine_record
  join pg_namespace namespace_record on namespace_record.oid = routine_record.pronamespace
  where namespace_record.nspname in ('public', 'storage', 'auth')
...
select md5(coalesce(
  jsonb_agg(jsonb_build_array(kind, identity, definition) order by kind, identity)::text,
  '[]'
