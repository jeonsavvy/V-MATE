...
begin
  ...
  join public.profiles profile on profile.user_id = owner.user_id
  where profile.age_confirmed_at is not null
  order by owner.created_at asc
  limit 1;

  if v_owner is null then
    raise exception 'A 17+ confirmed owner_users row is required before publishing starter content.';
  end if;
  ...
  (
    ...
    array['서브컬처', 'SF', '밝은성격', '성인'],
    jsonb_build_object('age', 21, 'occupation', '아카데미 기술부', 'creatorName', 'V-MATE'),
  ...
  (
    ...
    'public', 'visible', 'original', null, timezone('utc'::text, now()),
    array['서브컬처', 'SF판타지', '냉정', '성인'],
    jsonb_build_object('age', 24, 'occupation', '성간 항로 안내인', 'creatorName', 'V-MATE'),
